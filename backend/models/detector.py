...
class HateSpeechDetector:
    """Hate speech detection model wrapper"""
    ...
    def analyze(self, text):
        ...
        rule_is_hate, rule_conf = self.rule_based_detection(text)

        # ML prediction (if available)
        ml_is_hate, ml_conf = (False, 0.0)
        if self.model_loaded:
            try:
                ml_is_hate, ml_conf = self.predict_with_model(text)
        ...
        category = categorize_hate_speech(text) if is_hate else 'none'
